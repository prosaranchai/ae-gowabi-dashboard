...
def check_access():
    ...
    st.markdown("""
    <div style="max-width:360px;margin:100px auto 0;padding:36px;background:#fff;
                border:1px solid #ebebeb;border-radius:12px;box-shadow:0 4px 24px rgba(0,0,0,.06);
                text-align:center">
      <div style="font-size:32px;margin-bottom:8px">📊</div>
      <div style="font-size:18px;font-weight:700;letter-spacing:-.02em;margin-bottom:4px">AE Portfolio Deepdive</div>
      <div style="font-size:12px;color:#aaa;margin-bottom:24px">Gowabi — Internal use only</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col:
        key_input = st.text_input("Access Key", type="password",
                                   placeholder="ใส่ key เพื่อเข้าใช้งาน",
                                   key="access_key_input",
                                   label_visibility="collapsed")
        if st.button("เข้าใช้งาน", type="primary", use_container_width=True):
            access_key = st.secrets.get("ACCESS_KEY", st.secrets.get("ADMIN_PASSWORD", "gowabi2024"))
            if key_input == access_key:
                st.session_state["access_granted"] = True
                st.rerun()
            else:
                st.error("Key ไม่ถูกต้อง")
    return False
...
def process_raw(file_bytes: bytes, view_bytes: bytes | None = None) -> dict:
    ...
    for period in months_found:
        ...
        # Add sku_id as first sku_id per service (for display)
        svc_sku_id = mdf.groupby(["shop_id_s","service_name"])["sku_id"].first().reset_index()
        svc_sku_id["sku_id"] = svc_sku_id["sku_id"].astype(str).str.replace(".0","",regex=False).str.strip()
        ...
        svc_perf = svc_perf.merge(svc_sku_id, on=["shop_id_s","service_name"], how="left")
        ...
        result["months"][mkey] = {
            ...
            "svc_perf":    svc_perf[["shop_id_s","organization_name","kam","category",
                                      "service_name","sku_id","gmv","orders","basket",
            ...
            if len(cur_am) == 0:
                ...
                def get_sku_breakdown(shop_id, n=5):
                    ...
                    for _, sc_row in c.iterrows():
                        ...
                        _sku_id = str(sc_row.get("sku_id","")).replace("nan","").strip()
                        rows.append({
                            "svc":       svc,
                            "sku_id":    _sku_id,
                ...
                def render_sku_breakdown(skus, is_growth=True):
                    ...
                    def svc_row(s, show_sign=True):
                        sku_id_badge = (f' <span style="font-size:8px;background:#f5f5f5;color:#999;padding:1px 5px;border-radius:3px;font-family:monospace">#{s.get("sku_id","")}</span>' if s.get("sku_id") else "")
                        ...
                        return (f'<div style="display:flex;justify-content:space-between;align-items:center;'
                                ...
                                f'<span style="color:#555;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{name}{sku_id_badge}{new_b}</span>'
                        ...
                        if n_total > 0:
                            ...
                            for _ps in sorted(existing_skus, key=lambda x: (x["c_sell_min"]-x["p_sell_min"]), reverse=True):
                                try:
                                    ...
                                    pass
                            price_badge_str = f'<span style="color:{badge_col};font-weight:600">▾ {badge_txt}</span>'
                            price_sku_str   = price_badge_str  # kept for compat
                        else:
                            price_badge_str = ""
                            price_rows_html = ""
                            price_sku_str   = ""
...
<div style="background:#fff;border:1px solid #e8e8e8;border-radius:8px;padding:12px 16px;margin-bottom:8px;page-break-inside:avoid">
  ...
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-bottom:8px">
    ...
    <div style="background:#fafafa;border-radius:6px;padding:7px 10px;text-align:center">
      ...
      <div id="price_badge_{rank}" style="font-size:9px;cursor:pointer" onclick="var d=document.getElementById('price_detail_{rank}');d.style.display=d.style.display==='none'?'block':'none'">{price_badge_str}</div>
      <div id="price_detail_{rank}" style="display:none;text-align:left;margin-top:4px;max-height:180px;overflow-y:auto">{price_rows_html}</div>
    </div>
  </div>
